    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="bg-white shadow-md rounded-lg p-6">
            <div class="mb-6 border-b border-gray-200 pb-4">
                <h2 class="text-2xl font-semibold text-gray-800">Activity Levels</h2>
            </div>
            ...
            <!-- Form to add new activity level -->
            <div class="mb-10">
                <div class="bg-gray-50 rounded-lg shadow p-6 max-w-xl">
                    <h3 class="text-xl font-semibold mb-4 text-gray-700">Add New Activity Level</h3>
                    <form action="{{ route('admin.activity-levels.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
            ...
            <!-- List of existing activity levels -->
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 text-sm text-gray-700">
                        @forelse($activityLevels as $level)
                            <tr>
                                ...
                                <td class="px-4 py-2">{{ $level->description }}</td>
                                <td class="px-4 py-2">
                                    <form action="{{ route('admin.activity-levels.destroy', $level) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this activity level?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-4 text-center text-gray-500">No activity levels found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
