    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="bg-white shadow-md rounded-lg p-6">
            <div class="mb-6 border-b border-gray-200 pb-4">
                <h2 class="text-2xl font-semibold text-gray-800">Allergies</h2>
            </div>
            ...
            <div class="mb-10">
                <div class="bg-gray-50 rounded-lg shadow p-6 max-w-xl">
                    <h3 class="text-xl font-semibold mb-4 text-gray-700">Add New Allergy</h3>
                    <form action="{{ route('admin.allergies.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                ...
                <table class="min-w-full divide-y divide-gray-200 border text-sm text-left">
                    <thead class="bg-gray-100 text-gray-600 uppercase text-xs tracking-wider">
                        <tr>
                            ...
                            <th class="px-4 py-2">Allergy Name</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($allergies as $allergy)
                            <tr>
                                <td class="px-4 py-2">{{ $allergy->id }}</td>
                                <td class="px-4 py-2">{{ $allergy->name }}</td>
                                <td class="px-4 py-2">
                                    <form action="{{ route('admin.allergies.destroy', $allergy) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this allergy?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-4 text-center text-gray-500">No allergies found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
